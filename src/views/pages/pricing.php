<div class="relative font-sans pt-navbar-offset pb-0">
  ...
  <div class="container relative z-10 mx-auto px-6 max-w-[1550px]">
    <div class="text-center mb-16">
      ...
      <button id="btn-onprem" class="w-1/2 text-muted-foreground hover:text-foreground px-4 py-2 rounded-full font-bold text-sm sm:text-base transition-all duration-300 cursor-pointer">
        On-Premises
      </button>
    ...
    <div id="cloud-pricing-container" class="grid grid-cols-1 <?= $cloudGridClass ?> gap-6 lg:gap-8 mb-32 mx-auto transition-opacity duration-300">
      ...
      ?>
        <div class="pricing-card bg-card rounded-2xl p-6 md:p-10 flex flex-col relative <?= $isHighlighted ? 'border border-accent shadow-[0_0_25px_rgba(var(--accent-rgb),0.15)]' : 'border border-border' ?>">
          ...
          <div class="flex flex-col mb-6 gap-2">
            ...
            <div class="text-left">
              <div class="text-foreground text-xs font-semibold"><?= e($tier['idealForTitle'] ?? 'Ideal for') ?></div>
              <div class="text-muted-foreground text-[10px] sm:text-xs leading-tight whitespace-pre-line text-left"><?= e($tier['idealForText']) ?></div>
            </div>
            <?php endif; ?>
          </div>

          <h2 class="text-2xl md:text-3xl text-foreground font-bold leading-none mb-8">
            <?= e($tier['price']) ?>
          </h2>
          
          <ul class="space-y-3 mb-10 flex-1">
            <?php foreach ($tier['features'] as $f): ?>
              <li class="flex items-start gap-3 text-muted-foreground text-sm md:text-base">
    ...
    <div id="onprem-pricing-container" class="hidden grid grid-cols-1 <?= $onPremGridClass ?> gap-6 lg:gap-8 mb-32 mx-auto transition-opacity duration-300">
      ...
      ?>
        <div class="pricing-card bg-card rounded-2xl p-6 md:p-10 flex flex-col relative <?= $isHighlighted ? 'border border-accent shadow-[0_0_25px_rgba(var(--accent-rgb),0.15)]' : 'border border-border' ?>">
          ...
          <div class="text-foreground font-semibold text-base mb-4 min-h-[1.5rem]">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-10">
        <?php foreach ($addons as $addon): ?>
          <div class="pro-card transition-all duration-300 flex flex-col items-center text-center justify-center min-h-[200px]">
            ...
            <p class="text-muted-foreground text-sm md:text-base leading-relaxed whitespace-pre-line max-w-[95%]"><?= e($addon['description']) ?></p>
          </div>
        <?php endforeach; ?>
      </div>
    </div>
        ...
        <?php foreach ($deploymentOptions as $opt): ?>
          <div class="flex flex-col md:flex-row gap-8 md:gap-16 items-center">
             ...
             <div class="flex flex-col gap-3 text-center md:text-left">
                <h3 class="text-foreground text-xl md:text-2xl leading-tight font-bold">
                 <?= e($opt['name']) ?>
                 <?php if ($opt['subtitle']): ?>
                    <span class="text-muted-foreground text-sm md:text-base font-medium block mt-1">(<?= e($opt['subtitle']) ?>)</span>
                 <?php endif; ?>
               </h3>
               <p class="text-muted-foreground text-sm md:text-base leading-relaxed max-w-4xl"><?= e($opt['description']) ?></p>
             </div>
          </div>
        <?php endforeach; ?>
      </div>
  ...
  <div class="h-48 bg-gradient-to-b from-transparent to-background relative z-10 w-full pointer-events-none"></div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  btnCloud.addEventListener('click', function() {
    ...
    btnOnprem.className = "w-1/2 text-muted-foreground hover:text-foreground px-4 py-2 rounded-full font-bold text-sm sm:text-base transition-all duration-300 cursor-pointer";
  ...
  btnOnprem.addEventListener('click', function() {
    ...
    btnCloud.className = "w-1/2 text-muted-foreground hover:text-foreground px-4 py-2 rounded-full font-bold text-sm sm:text-base transition-all duration-300 cursor-pointer";
